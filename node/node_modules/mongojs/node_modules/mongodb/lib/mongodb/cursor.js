var QueryCommand = require('./commands/query_command')
  .QueryCommand,
  GetMoreCommand = require('./commands/get_more_command')
    .GetMoreCommand,
  KillCursorCommand = require('./commands/kill_cursor_command')
    .KillCursorCommand,
 ...
 */
function Cursor(db, collection, selector, fields, skip, limit, sort, hint, explain, snapshot, timeout, tailable, batchSize, slaveOk, raw, read, returnKey, maxScan, min, max, showDiskLoc, comment) {
  this.db = db;
...
var generateQueryCommand = function(self) {
  ...
  if (self.sortValue != null || self.explainValue != null || self.hint != null || self.snapshot != null || self.returnKey != null || self.maxScan != null || self.min != null || self.max != null || self.showDiskLoc != null || self.comment != null) {
...
Cursor.prototype.nextObject = function(callback) {
  ...
  if (self.state == Cursor.INIT) {
    ...
    self.db._executeQueryCommand(cmd, {
      read: self.read,
      raw: self.raw
    }, commandHandler);
    commandHandler = null;
...
var getMore = function(self, callback) {
  ...
  try {
    ...
    // Execute the command
    self.db._executeQueryCommand(getMoreCommand, {
      read: self.read,
      raw: self.raw
    }, function(err, result) {
      try {
...
Cursor.prototype.streamRecords = function(options) {
  ...
  function execute(command) {
    self.db._executeQueryCommand(command, {
      read: self.read,
      raw: self.raw
    }, function(err, result) {
      if (err) {
