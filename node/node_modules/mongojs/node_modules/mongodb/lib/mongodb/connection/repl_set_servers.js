var Connection = require('./connection')
  .Connection,
  DbCommand = require('../commands/db_command')
    .DbCommand,
  MongoReply = require('../responses/mongo_reply')
    .MongoReply,
  debug = require('util')
    .debug,
  ...
  format = require('util')
    .format,
  PingStrategy = require('./strategies/ping_strategy')
    .PingStrategy,
  StatisticsStrategy = require('./strategies/statistics_strategy')
    .StatisticsStrategy;
...
var ReplSetServers = exports.ReplSetServers = function(servers, options) {
  ...
  if (readPreference != null) {
    if (readPreference != Server.READ_PRIMARY && readPreference != Server.READ_SECONDARY_ONLY && readPreference != Server.READ_SECONDARY) {
  ...
  this.logger = this.options.logger != null && (typeof this.options.logger.debug == 'function') && (typeof this.options.logger.error == 'function') && (typeof this.options.logger.debug == 'function') ? this.options.logger : {
  ...
  // Db Instances
  Object.defineProperty(this, "dbInstances", {
    enumerable: true,
    get: function() {
  ...
  Object.defineProperty(this, "allSecondaries", {
    enumerable: true,
    get: function() {
...
ReplSetServers.prototype.setReadPreference = function(preference) {
  ...
  if ((this._readPreference == Server.READ_SECONDARY || this._readPreference == Server.READ_SECONDARY_ONLY) || (this._readPreference != null && typeof this._readPreference == 'object')) {
    this.slaveOk = true;
  }
}
...
var cleanupTags = ReplSetServers._cleanupTags = function(server, byTags) {
  ...
  for (var i = 0; i < serverTagKeys.length; i++) {
    ...
    if (byTags[serverTagKeys[i]] != null && byTags[serverTagKeys[i]][value] != null && Array.isArray(byTags[serverTagKeys[i]][value])) {
...
ReplSetServers.prototype.connect = function(parent, options, callback) {
  ...
  var connectionHandler = function(instanceServer) {
    return function(err, result) {
        ...
        if (isMaster == true || secondary == true || arbiterOnly == true) {
          ...
          var closeHandler = function(err, server) {
            ...
            if (replSetSelf._state.master != null) {
              ...
              if (primaryAddress == errorServerAddress) {
                ...
                if (replSetSelf._readPreference == Server.READ_SECONDARY_ONLY && Object.keys(replSetSelf._state.secondaries)
                  .length == 0) {
                  closeServers();
                }
              }
          ...
          for (var i = 0; i < candidateServers.length; i++) {
            ...
            if (replSetSelf._state.addresses[candidateServerString] == null) {
              ...
              var newServer = new Server(parts[0], parseInt(parts[1]), {
                auto_reconnect: false,
                'socketOptions': socketOptions,
                logger: replSetSelf.logger,
                ssl: replSetSelf.ssl,
                poolSize: replSetSelf.poolSize
              });
              ...
              newServer.connect(parent, {
                returnIsMasterResults: true,
                eventReceiver: newServer
              }, connectionHandler(newServer));
            }
          }
        } else {
      ...
      if ((numberOfServersLeftToInitialize == 0) && replSetSelf._serverState === 'connecting' && replSetSelf._state.errorMessages.length == 0) {
        ...
        } else if (replSetSelf.readSecondary == true && Object.keys(replSetSelf._state.secondaries)
          .length > 0) {
          // If we have a read strategy boot it
          if (replSetSelf.strategyInstance != null) {
            // Ensure we have a proper replicaset defined
  ...
  for (var i = 0; i < serverConnections.length; i++) {
    ...
    // Connect to server
    serverConnections[i].connect(parent, {
      returnIsMasterResults: true,
      eventReceiver: serverConnections[i]
    }, connectionHandler(serverConnections[i]));
  }
...
ReplSetServers.prototype.checkoutReader = function() {
  ...
  // If we have specified to read from a secondary server grab a random one and read
  // from it, otherwise just pass the primary connection
  if ((this.readSecondary == true || this._readPreference == Server.READ_SECONDARY || this._readPreference == Server.READ_SECONDARY_ONLY) && Object.keys(this._state.secondaries)
    .length > 0) {
    // Checkout a secondary server from the passed in set of servers
    if (this.strategyInstance != null) {
  ...
  } else if (this._readPreference == Server.READ_SECONDARY_ONLY && Object.keys(this._state.secondaries)
    .length == 0) {
    connection = null;
  } else if (this._readPreference != null && typeof this._readPreference === 'object') {
    // Get all tag keys (used to try to find a server that is valid)
    ...
    for (var i = 0; i < keys.length; i++) {
      ...
      // Check if we have any servers for the tag, if we do pick a random one
      if (this._state.byTags[keys[i]] != null && this._state.byTags[keys[i]][value] != null && Array.isArray(this._state.byTags[keys[i]][value]) && this._state.byTags[keys[i]][value].length > 0) {
        // Let's grab an available server from the list using a random pick
        var serverInstances = this._state.byTags[keys[i]][value];
        // Set instance to return
        instanceServer = serverInstances[Math.floor(Math.random() * serverInstances.length)];
...
ReplSetServers.prototype.allRawConnections = function() {
  ...
  if (this.readSecondary && Object.keys(this._state.secondaries)
    .length > 0) {
    // Get all the keys
    var keys = Object.keys(this._state.secondaries);
