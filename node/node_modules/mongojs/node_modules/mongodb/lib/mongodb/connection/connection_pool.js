...
var _connect = function(_self) {
  return new function() {
    ...
    connection.on("error", function(err, connection) {
      ...
      if (connectionStatus == 'disconnected' && _self.listeners("error")
        .length > 0) {
    ...
    connection.on("close", function() {
      // If we are already disconnected ignore the event
      if (connectionStatus !== 'disconnected' && _self.listeners("close")
        .length > 0) {
        _self.emit("close");
      }
    ...
    connection.on("timeout", function(err, connection) {
      // If we are already disconnected ignore the event
      if (connectionStatus !== 'disconnected' && _self.listeners("timeout")
        .length > 0) {
    ...
    connection.on("parseError", function() {
      // If we are already disconnected ignore the event
      if (connectionStatus !== 'disconnected' && _self.listeners("parseError")
        .length > 0) {
        // if(connectionStatus == 'connected') {
