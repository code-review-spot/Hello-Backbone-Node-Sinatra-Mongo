...
var QueryCommand = require('./commands/query_command')
  ...
  DbCommand = require('./commands/db_command')
    .DbCommand,
  BinaryParser = require('./bson/binary_parser')
    .BinaryParser,
  ...
  Server = require('./connection/server')
    .Server,
  ReplSetServers = require('./connection/repl_set_servers')
    .ReplSetServers,
...
function Db(databaseName, serverConfig, options) {
  ...
  // Set up logger
  this.logger = this.options.logger != null && (typeof this.options.logger.debug == 'function') && (typeof this.options.logger.error == 'function') && (typeof this.options.logger.log == 'function') ? this.options.logger : {
  ...
  this._lastReaperTimestamp = new Date()
    .getTime();
...
Db.prototype.open = function(callback) {
  ...
  if (self.serverConfig instanceof Server || self.serverConfig instanceof ReplSetServers) {
    self.serverConfig.connect(self, {
      firstCall: true
    }, function(err, result) {
      if (err != null) {
...
Db.prototype.logout = function(callback) {
  ...
  var logoutCommand = DbCommand.logoutCommand(self, {
    logout: 1
  });
  self._executeQueryCommand(logoutCommand, {
    onAll: true
  }, function(err, result) {
    // Reset auth
    self.auths = [];
...
Db.prototype.authenticate = function(username, password, callback) {
  ...
  var numberOfConnections = this.serverConfig.allRawConnections()
    .length;
  var errorObject = null;

  // Execute all four
  this._executeQueryCommand(DbCommand.createGetNonceCommand(self), {
    onAll: true
    ...
    if (err == null) {
      ...
      // Execute command
      self._executeQueryCommand(DbCommand.createAuthenticationCommand(self, username, password, nonce), {
...
Db.prototype.createIndex = function(collectionName, fieldOrSpec, options, callback) {
  ...
  this._executeInsertCommand(command, {
    read: false,
    safe: true
  }, function(err, result) {
    if (err != null) return callback(err, null);
...
Db.prototype.ensureIndex = function(collectionName, fieldOrSpec, options, callback) {
  ...
  this.indexInformation(collectionName, function(err, collectionInfo) {
    ...
    if (!collectionInfo[index_name]) {
      self._executeInsertCommand(command, {
        read: false,
        safe: true
      }, function(err, result) {
...
Db.prototype.cursorInfo = function(callback) {
  this._executeQueryCommand(DbCommand.createDbCommand(this, {
    'cursorInfo': 1
  }), function(err, result) {
...
Db.prototype.indexInformation = function(collectionName, options, callback) {
  ...
  var selector = collectionName != null ? {
    ns: (this.databaseName + "." + collectionName)
  } : {};
  // Iterate through all the fields of the index
  new Cursor(this, new Collection(this, DbCommand.SYSTEM_INDEX_COLLECTION), selector)
...
Db.prototype._registerHandler = function(db_command, raw, connection, callback) {
  ...
  if (chained) {
    ...
    var chainedIds = [];
    // Add all id's 
    for (var i = 0; i < db_command.length; i++) chainedIds.push(db_command[i].getRequestId()
      .toString());
    ...
    for (var i = 0; i < db_command.length; i++) {
      ...
      this._callBackStore._notReplied[command.getRequestId()
        .toString()] = {
        start: new Date()
          .getTime(),
        'raw': raw,
    ...
    // Add the information about the reply
    this._callBackStore._notReplied[db_command.getRequestId()
      .toString()] = {
      start: new Date()
        .getTime(),
      'raw': raw,
...
Db.prototype._callHandler = function(id, document, err) {
  ...
  if (this._callBackStore.listeners(id)
    .length >= 1) {
    // Get info object
...
Db.prototype._hasHandler = function(id) {
  // If there is a callback peform it
  return this._callBackStore.listeners(id)
    .length >= 1;
}
...
Db.prototype._findHandler = function(id) {
  ...
  return {
    info: info,
    callback: (this._callBackStore.listeners(id)
      .length >= 1)
  }
}
