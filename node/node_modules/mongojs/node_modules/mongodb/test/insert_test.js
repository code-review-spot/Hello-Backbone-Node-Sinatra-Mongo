...
var testCase = require('../deps/nodeunit')
  ...
  ObjectID = require('../lib/mongodb/bson/objectid')
    .ObjectID,
  Binary = require('../lib/mongodb/bson/binary')
    .Binary,
  Code = require('../lib/mongodb/bson/code')
    .Code,
  DBRef = require('../lib/mongodb/bson/db_ref')
    .DBRef,
  Timestamp = require('../lib/mongodb/bson/timestamp')
    .Timestamp,
...
exports["Should correctly execute insert with keepGoing option on mongod >= 1.9.1"] = function(test) {
  ...
  db.open(function(err, db) {
    ...
    db.admin()
      .serverInfo(function(err, result) {
...
exports.shouldForceMongoDbServerToAssignId = function(test) {
  ...
  var db = new Db(MONGODB, new Server('localhost', 27017, {
    auto_reconnect: true,
    ssl: useSSL
  }), {
    native_parser: (process.env['TEST_NATIVE'] != null),
...
exports.shouldCorrectlyInsertAndRetrieveDocumentWithAllTypes = function(test) {
  client.createCollection('test_all_serialization_types', function(err, collection) {
    ...
    collection.insert(motherOfAllDocuments, {
      safe: true
    }, function(err, docs) {
  ...
  db.open(function(err, db) {
    ...
    db.collection('users', getResult(function(user_collection) {
      user_collection.remove({}, {
        ...
        user_collection.insert([newUser], {
          safe: true
        }, getResult(function(users) {
          var user = users[0];
...
exports.shouldCorrectlySerializeDocumentWithAllTypesInNewContext = function(test) {
  client.createCollection('test_all_serialization_types_new_context', function(err, collection) {
    ...
    collection.insert(context.motherOfAllDocuments, {
      safe: true
    }, function(err, docs) {
      collection.findOne(function(err, doc) {
        // Assert correct deserialization of the values
...
exports.shouldCorrectlyPerformSafeInsert = function(test) {
  var fixtures = [{
    ...
    float: 1.5,
    string: "something simply nested"
    }];
...
exports['Should Correctly allow for using a Date object as _id'] = function(test) {
  ...
  client.createCollection("Should_Correctly_allow_for_using_a_Date_object_as__id", {
    serializeFunctions: true
  }, function(err, collection) {
    test.ok(err == null);
...
exports['Should Correctly fail to update returning 0 results'] = function(test) {
  client.createCollection("Should_Correctly_fail_to_update_returning_0_results", {
    serializeFunctions: true
  }, function(err, collection) {
    test.ok(err == null);
