var debug = require('util')
  ...
  spawn = require('child_process')
    .spawn,
  Connection = require('../../lib/mongodb')
    .Connection,
  Db = require('../../lib/mongodb')
    .Db,
  Server = require('../../lib/mongodb')
    .Server,
    ...
    Step(
      function startAllServers() {
        ...
        for (n = 0; n < (self.primaryCount + self.secondaryCount); n++) {
          self.initNode(n, {
            tags: self.tags[tagsIndex] != null ? self.tags[tagsIndex++] : null
          }, group());
        }
  ...
  exec("rm -rf " + self.mongods[n]["db_path"], function(err, stdout, stderr) {
    ...
    exec("mkdir -p " + self.mongods[n]["db_path"], function(err, stdout, stderr) {
      ...
      self.start(n, function() {
        ...
        var member = {
          "_id": n,
          "host": self.host + ":" + self.mongods[n]["port"]
        };
...
ReplicaSetManager.prototype.getNodeFromPort = function(port, callback) {
  var self = this;
  var nodes = Object.keys(this.mongods)
...
ReplicaSetManager.prototype.getNodeWithState = function(state, callback) {
  ...
  self.ensureUp(function(err, status) {
    ...
    if (node != null) {
      ...
      var key = Object.keys(self.mongods)
...
ReplicaSetManager.prototype.restartKilledNodes = function(callback) {
  ...
  var nodes = Object.keys(self.mongods)
    .filter(function(key) {
...
var start = ReplicaSetManager.prototype.start = function(node, callback) {
  ...
  setTimeout(function() {
    ...
    self.mongods[node]["pid"] = fs.readFileSync(path.join(self.mongods[node]["db_path"], "mongod.lock"), 'ascii')
      .trim();
    // Callback
    callback();
  }, 5000);
