var ReplicaSetManager = require('../../test/tools/replica_set_manager')
  .ReplicaSetManager;
...
var mongo = require('../../lib/mongodb'),
  ...
  accounts = [],
  accountsCollection, contentCollection, count = 0;
...
RS.startSet(true, function(err, result) {
  ...
  var replSet = new mongo.ReplSetServers([
      new mongo.Server(RS.host, RS.ports[1], {
      auto_reconnect: true
    }),
      new mongo.Server(RS.host, RS.ports[0], {
      auto_reconnect: true
    }),
      new mongo.Server(RS.host, RS.ports[2], {
      auto_reconnect: true
    })
    ], {
    rs_name: RS.name,
  ...
  function loadContent() {
    ...
    contentCollection.find(criteria, {
      fields: fields,
      limit: 100000,
      sort: sort
    }, function(err, cursor) {
  ...
  website.open(function(err, wsClient) {
    ...
    adserver.open(function(err, asClient) {
      ...
      accountsCollection.insert(accountDocs, {
        safe: {
          w: 2,
          wtimout: 1000
        }
      }, function(err, r) {
        contentCollection.insert(contentDocs, {
          safe: {
            w: 2,
            wtimout: 1000
          }
