...
RS.startSet(true, function(err, result) {
  ...
  var replSet = new mongodb.ReplSetServers([
      new mongodb.Server(RS.host, RS.ports[1], {
      auto_reconnect: true
    }),
      new mongodb.Server(RS.host, RS.ports[0], {
      auto_reconnect: true
    }),
      new mongodb.Server(RS.host, RS.ports[2], {
      auto_reconnect: true
    })
    ], {
    rs_name: RS.name,
    read_secondary: true,
    readPreference: mongodb.Server.READ_SECONDARY
  });
  ...
  db.open(function(err, client) {
    ...
    // Insert all the docs
    userCollection.insert(userDocs, {
      safe: true
    }, function(err, result) {
      console.dir(err);

      accountCollection.insert(accountDocs, {
        safe: true
      }, function(err, result) {
        console.dir(err);
  ...
  function lookup(cb) {
    ...
    db.collection('users', function(err, userCollection) {
      ...
      userCollection.findOne({
        user_id: Math.floor(Math.random(maxUserId))
      }, function(err, user) {
